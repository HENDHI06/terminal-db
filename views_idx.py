...
def render_dashboard_utama():
    st.markdown(f"<h2 class='gradient-text' style='margin-bottom:5px;'>Dashboard Saham IDX</h2>", unsafe_allow_html=True)
    ...
    try:
        ...
        if not ihsg_data.empty and len(ihsg_data) >= 2:
            ...
            ihsg_color = "#34D399" if ihsg_pct > 0 else "#EF4444"
            ...
            st.markdown(f"""<div class='dash-box' style='border-left: 4px solid {ihsg_color}; border-top: 1px solid #E2E8F0 !important; padding: 20px; margin-bottom: 0px !important;'>
                <p class='text-muted' style='margin:0; font-weight:600;'>IHSG (INDEKS HARGA SAHAM GABUNGAN)</p>
                ...
                <p style='margin:0; font-size:14px; color:#F8FAFC;'>Status Pasar Terakhir: <span class='{badge_ihsg}'>{ihsg_status}</span></p>
            ...
            if total_valid > 0:
                st.markdown(f"""<div class='dash-box' style='padding: 20px;'>
                    ...
                    <div style='display:flex; justify-content:space-around;'>
                        ...
                        <div style='text-align:center;'><h2 style='margin:0; color:#94A3B8;'>{flat}</h2><span class='text-muted'>Mandek ➖</span></div>
    ...
    with st.spinner("Melacak Sentimen dan Asing..."):
        try:
            ...
            flow_color = "#34D399" if net_flow > 0 else "#EF4444"
            ...
            st.markdown(f"""<div class='dash-box' style='border-top: 3px solid {flow_color} !important; text-align:center; padding: 20px;'>
                ...
                <p style='font-size:13px; color:#F8FAFC;'>Indikator Kekuatan: <b>{net_flow:.2f}</b></p>
            </div>""", unsafe_allow_html=True)
        except: pass
        ...
        try:
            ...
            elif fg_value <= 70: fg_status, fg_color = "GREED", "#34D399"
            else: fg_status, fg_color = "EXTREME GREED", "#10B981"
            ...
            fig_fg = go.Figure(go.Indicator(
                ...
                gauge = {
                    ...
                    'steps': [
                        {'range': [0, 30], 'color': "rgba(239, 68, 68, 0.2)"}, {'range': [30, 45], 'color': "rgba(245, 158, 11, 0.2)"},
                        {'range': [45, 55], 'color': "rgba(56, 189, 248, 0.2)"}, {'range': [55, 75], 'color': "rgba(52, 211, 153, 0.2)"},
            ...
            try:
                ...
                if not df_val.empty:
                    for _, row in df_val.iterrows():
                        st.markdown(f"<div class='dash-box' style='border-left: 4px solid #34D399; padding: 14px;'><b style='font-size:16px; color:#F8FAFC;'>{row['Ticker']}</b> <span class='badge-green' style='float:right;'>Trx: Rp {row['Value']/1e9:,.1f} Miliar 💵</span></div>", unsafe_allow_html=True)
...
def render_auto_scanner():
    ...
    if st.session_state.results_saham is not None and not st.session_state.results_saham.empty:
        ...
        with tab2:
            st.markdown("<h4 style='color:#38BDF8;'>Peta Dominasi Perputaran Uang</h4>", unsafe_allow_html=True)
            st.caption("Semakin besar kotaknya, semakin besar uang yang berputar. Warna Hijau = Naik, Merah = Turun.")
            fig_tree = px.treemap(df, path=[px.Constant("Bursa"), 'SEKTOR', 'TICKER'], values='VAL(M)', color='CHG%', color_continuous_scale=["#EF4444", "#1E293B", "#34D399"], color_continuous_midpoint=0)
            fig_tree.update_layout(template="plotly_dark", margin=dict(t=10, l=10, r=10, b=10), paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
            st.plotly_chart(fig_tree, use_container_width=True)
            
        with tab3: 
            # --- MAP PANDAS BARU (ANTI ERROR) ---
            ...
            styled_df = df.drop(columns=['FULL', 'SCORE_MOM', 'SCORE_BNDR', 'SCORE_TRND', 'SCORE_VOL'], errors='ignore').style.format(format_mapping).map(style_dataframe).apply(highlight_cols)
        ...
        with tab4:
            ...
            with c_rad:
                ...
                fig_radar.add_trace(go.Scatterpolar(
                    ...
                    theta=categories, fill='toself', name=sel_t, marker_color='#38BDF8'
                ))
            ...
            if not c_data.empty:
                ...
                fig = make_subplots(rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.03, row_heights=[0.7, 0.3])
                fig.add_trace(go.Candlestick(x=c_data.index, open=c_data['Open'], high=c_data['High'], low=c_data['Low'], close=c_data['Close'], increasing_line_color='#34D399', decreasing_line_color='#EF4444', name='Price'), row=1, col=1)
                fig.add_trace(go.Scatter(x=c_data.index, y=c_data['MA20'], line=dict(color='#38BDF8', width=1.5), name='MA 20'), row=1, col=1)
                fig.add_trace(go.Scatter(x=c_data.index, y=c_data['MA50'], line=dict(color='#FBBF24', width=1.5), name='MA 50'), row=1, col=1)
                colors = ['#34D399' if row['Close'] >= row['Open'] else '#EF4444' for index, row in c_data.iterrows()]
...
def render_strategy_scanner():
    st.markdown(f"<h2 class='gradient-text'>⚡ Institutional Strategy Scanner</h2>", unsafe_allow_html=True)
    with st.expander("📖 PANDUAN STRATEGI PRO (WAJIB BACA)", expanded=False):
        st.markdown("""
        **1. Minervini Trend Template:** Mencari saham super yang sedang dalam Fase 2 (Mark-Up) ala Juara Trading US.
        **2. Relative Strength (Alpha):** Mencari saham yang melawan arus (naik saat IHSG turun).
        **3. VCP (Volatility Contraction):** Mencari saham yang harganya menyempit dan volume kering (siap meledak).
        **4. Fair Value Gap (FVG):** Mencetak area "Magnet" (Order Block) tempat institusi meninggalkan celah harga.
        **5. MA Crossover:** Sinyal standar pembalikan arah tren.
        """)
        
    try:
        df_saham = pd.read_excel("daftar_saham.xlsx")
        # Batasi 100 koin agar scanner tidak memberatkan server dan gagal muat (timeout)
        watchlist = [t.strip() + ".JK" for t in df_saham['Kode'].dropna().astype(str).tolist()][:100] 
    except:
        watchlist = load_tickers()[:100]

    tab1, tab2, tab3, tab4, tab5 = st.tabs(["🚀 MINERVINI", "💪 RELATIVE STRENGTH", "🗜️ VCP SQUEEZE", "🧲 FVG GAP", "⚡ CROSSOVER"])
    
    with tab1:
        st.info("Scanner Super Momentum ala Mark Minervini. Hanya mencari saham elit yang sedang tren naik kuat.")
        if st.button("Pindai Minervini", use_container_width=True):
            with st.spinner("Mengukur tren jangka panjang 100 Emiten..."):
                res = run_pro_scanner(watchlist, "Minervini")
                if not res.empty:
                    for _, r in res.iterrows(): st.markdown(f"<div class='dash-box'><span class='badge-green'>{r['Sinyal']}</span><p style='margin:8px 0; color:#F8FAFC;'>Saham: <b style='color:#38BDF8;'>{r['Saham']}</b> | Harga: Rp {r['Harga']:,.0f}<br><small style='color:#94A3B8;'>{r['Detail']}</small></p></div>", unsafe_allow_html=True)
                else: st.warning("Tidak ada saham yang memenuhi kriteria super ketat ini hari ini.")
                
    with tab2:
        st.info("Mencari saham dengan performa jauh meninggalkan IHSG (Alpha). Sangat bagus saat IHSG merah.")
        if st.button("Pindai Alpha Market", use_container_width=True):
            with st.spinner("Membandingkan performa dengan IHSG..."):
                res = run_pro_scanner(watchlist, "RS")
                if not res.empty:
                    for _, r in res.iterrows(): st.markdown(f"<div class='dash-box'><span class='badge-blue'>{r['Sinyal']}</span><p style='margin:8px 0; color:#F8FAFC;'>Saham: <b style='color:#38BDF8;'>{r['Saham']}</b> | Harga: Rp {r['Harga']:,.0f}<br><small style='color:#94A3B8;'>{r['Detail']}</small></p></div>", unsafe_allow_html=True)
                else: st.warning("Tidak ada saham alpha kuat hari ini.")
                
    with tab3:
        st.info("Volatility Contraction Pattern. Mencari pegas tertekan dengan volume mengering (Siap Breakout).")
        if st.button("Pindai VCP / Squeeze", use_container_width=True):
            with st.spinner("Mendeteksi penyempitan harga..."):
                res = run_pro_scanner(watchlist, "VCP")
                if not res.empty:
                    for _, r in res.iterrows(): st.markdown(f"<div class='dash-box'><span class='badge-green'>{r['Sinyal']}</span><p style='margin:8px 0; color:#F8FAFC;'>Saham: <b style='color:#38BDF8;'>{r['Saham']}</b> | Harga: Rp {r['Harga']:,.0f}<br><small style='color:#94A3B8;'>{r['Detail']}</small></p></div>", unsafe_allow_html=True)
                else: st.warning("Tidak ada pola penyempitan (VCP) terdeteksi.")
                
    with tab4:
        st.info("Mencari Order Block institusi (ketidakseimbangan harga) yang belum tersentuh.")
        if st.button("Pindai FVG Market", use_container_width=True):
            with st.spinner("Mencari jejak gap Smart Money..."):
                res = run_pro_scanner(watchlist, "FVG")
                if not res.empty:
                    for _, r in res.iterrows():
                        c_bdg = "badge-green" if "BULLISH" in r['Sinyal'] else "badge-red"
                        st.markdown(f"<div class='dash-box'><span class='{c_bdg}'>{r['Sinyal']}</span><p style='margin:8px 0; color:#F8FAFC;'>Saham: <b style='color:#38BDF8;'>{r['Saham']}</b> | Harga: Rp {r['Harga']:,.0f}<br><small style='color:#94A3B8;'>{r['Detail']}</small></p></div>", unsafe_allow_html=True)
                else: st.warning("Tidak ada FVG fresh hari ini.")
                
    with tab5:
        st.info("Sinyal persilangan Moving Average klasik (MA20 vs MA50).")
        if st.button("Pindai Crossover", use_container_width=True):
            with st.spinner(f"Menganalisis perpaduan Tren..."):
                ...
                if results:
                    ...
                    for res in results:
                        ...
                        st.markdown(f"<div class='dash-box'><span class='{bg_badge}' style='margin-bottom:8px;'>{res['status']}</span><p style='margin:8px 0 0 0; color:#F8FAFC;'>Saham: <b style='color:#38BDF8;'>{res['ticker']}</b> | Harga: Rp {res['price']:,.0f}</p></div>", unsafe_allow_html=True)
        ...
        with st.spinner("Menghitung kalkulasi Pivot Point..."):
            try:
                ...
                if not df_piv.empty and len(df_piv) >= 20:
                    ...
                    fig = go.Figure(data=[go.Candlestick(x=df_chart.index, open=df_chart['Open'], high=df_chart['High'], low=df_chart['Low'], close=df_chart['Close'], increasing_line_color='#34D399', decreasing_line_color='#EF4444', name='Harga')])
                    ...
                    fig.add_hline(y=s1, line_dash="solid", line_color="#34D399", annotation_text="S1"); fig.add_hline(y=s2, line_dash="dash", line_color="#34D399", annotation_text="S2")
                    fig.update_layout(template="plotly_dark", height=450, margin=dict(l=0,r=0,t=10,b=0), xaxis_rangeslider_visible=False, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
                    st.plotly_chart(fig, use_container_width=True)
        ...
        with st.spinner("Mengekstrak sejarah harga 5 tahun terakhir..."):
            try:
                ...
                if not df_season.empty:
                    ...
                    fig_season = px.bar(monthly_stats, x='Bulan', y='Win Rate (%)', color='Win Rate (%)', color_continuous_scale=["#EF4444", "#1E293B", "#34D399"], text_auto='.0f')
    ...
    if btn_analyze:
        ...
        with st.spinner("Memuat data laporan keuangan terakhir..."):
            try:
                ...
                st.markdown("---")
                st.markdown("<h4 style='color:#38BDF8;'>🧠 Analisis Valuasi Multi-Guru</h4>", unsafe_allow_html=True)
        ...
        if st.button("Pantau Pergerakan Harga", use_container_width=True):
            with st.spinner("Memetakan arus sektor..."):
                ...
                if sector_data:
                    ...
                    fig_sec = px.bar(df_sec, x="Sektor", y="Perubahan (%)", color="Perubahan (%)", color_continuous_scale=["#EF4444", "#1E293B", "#34D399"])
            ...
            with st.spinner("Mengkalkulasi kekuatan relatif terhadap IHSG..."):
                try:
                    ...
                    if rrg_data:
                        ...
                        fig_rrg.add_hline(y=100, line_dash="dash", line_color="#334155")
                        fig_rrg.add_vline(x=100, line_dash="dash", line_color="#334155")
                        fig_rrg.add_annotation(x=102, y=102, text="LEADING 🚀", showarrow=False, font=dict(color="#34D399", size=14))
                        fig_rrg.add_annotation(x=102, y=98, text="WEAKENING 📉", showarrow=False, font=dict(color="#FBBF24", size=14))
                        fig_rrg.add_annotation(x=98, y=98, text="LAGGING 🛑", showarrow=False, font=dict(color="#EF4444", size=14))
                        fig_rrg.add_annotation(x=98, y=102, text="IMPROVING 📈", showarrow=False, font=dict(color="#38BDF8", size=14))
                        
                        fig_rrg.update_traces(textposition='top center')
                        fig_rrg.update_layout(template="plotly_dark", height=500, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', showlegend=False)
            ...
            with st.spinner("Membongkar distribusi aliran..."):
                try:
                    ...
                    if not df_ff.empty and len(df_ff) > 20:
                        ...
                        fig_mf.add_hline(y=0, line_dash="dash", line_color="#334155")
                ...
                try:
                    ...
                    if not df_v.empty:
                        ...
                        if total_volume > 0:
                            ...
                            fig = go.Figure(data=[go.Candlestick(x=df_v.index, open=df_v['Open'], high=df_v['High'], low=df_v['Low'], close=df_v['Close'], increasing_line_color='#34D399', decreasing_line_color='#EF4444')])
            ...
            with st.spinner("Mengecek pergerakan di balik layar..."):
                try:
                    ...
                    if not df_div.empty and len(df_div) > 20:
                        ...
                        st.markdown(f"<div class='dash-box' style='text-align:center;'><br><span class='{badge_div}' style='font-size:1.1rem; padding:8px 16px;'>{status_div}</span><p style='font-size:14px; margin-top:15px; color:#F8FAFC;'>{desc}</p></div>", unsafe_allow_html=True)
                        
                        fig_div = make_subplots(rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.05, row_heights=[0.6, 0.4])
                        fig_div.add_trace(go.Candlestick(x=df_div.index, open=df_div['Open'], high=df_div['High'], low=df_div['Low'], close=df_div['Close'], increasing_line_color='#34D399', decreasing_line_color='#EF4444', name='Harga'), row=1, col=1)
        ...
        with st.spinner("Memindai berita sekuritas harian..."):
            try:
                ...
                for entry in feed.entries[:10]: 
                    ...
                    st.markdown(f"<div class='dash-box'><div style='display:flex; justify-content:space-between; margin-bottom:8px;'><span class='{badge_c}'>{sent_text}</span><span style='font-size:11px; color:#EF4444; font-weight:700;'>{fire_badge}</span></div><a href='{entry.link}' target='_blank' style='color:#38BDF8; text-decoration:none; font-size:1rem; font-weight:600;'>{entry.title}</a><p class='text-muted' style='margin-top:8px; margin-bottom:0;'>⏰ {pub_date}</p></div>", unsafe_allow_html=True)
        ...
        with st.spinner("Memindai almanak korporasi..."):
            try:
                ...
                for entry in feed_corp.entries[:10]: 
                    fire_badge = check_if_new(entry.published_parsed if hasattr(entry, 'published_parsed') else None)
                    pub_date = entry.published if hasattr(entry, 'published') else ""
                    st.markdown(f"<div class='dash-box'><div style='display:flex; justify-content:space-between; margin-bottom:8px;'><span class='badge-blue'>📅 INFO CORPORATE ACTION</span><span style='font-size:11px; color:#EF4444; font-weight:700;'>{fire_badge}</span></div><a href='{entry.link}' target='_blank' style='color:#38BDF8; text-decoration:none; font-size:1rem; font-weight:600;'>{entry.title}</a><p class='text-muted' style='margin-top:8px; margin-bottom:0;'>⏰ {pub_date}</p></div>", unsafe_allow_html=True)
