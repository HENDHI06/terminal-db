    ...
    for koin in target_koin:
        try:
            ...
            if not df_hist.empty:
                ...
                is_squeeze = bbw_current <= df_hist['BBW'].min() * 1.1 
...
def render_dasbor_indodax():
    ...
    fig_gauge = go.Figure(go.Indicator(
        mode = "gauge+number",
        value = fgi_val,
        title = {'text': f"Psikologi Massa: <b>{fgi_class.upper()}</b>", 'font': {'size': 18, 'color': 'white'}},
        gauge = {
...
def render_radar_altcoin():
    ...
    def hitung_skor(row):
        ...
        if koin in rvol_data:
            ...
            if rvol_val > 200: rvol_bonus = 20 
            elif rvol_val > 100: rvol_bonus = 10
            
        return min(vol_pts + bounce_pts + rvol_bonus, 100)
...
def render_whale_tracker():
                    ...
                    st.markdown(f"""
                        <div style="margin-bottom: 20px;">
                            ...
                            <div style="height: 15px; width: 100%; background: linear-gradient(90deg, #EF4444 0%, #E2E8F0 50%, #10B981 100%); border-radius: 10px; position: relative;">
                                <div style="position: absolute; top: -5px; left: {50 + (obi_score/2)}%; width: 5px; height: 25px; background: white; border-radius: 2px;"></div>
                            </div>
                        </div>
    ...
    with tab_liq:
        st.info("Mendeteksi *Funding Rate* Binance Global khusus untuk koin yang ada di Indodax.")
...
def render_dca():
    st.markdown("<h2 class='gradient-text'>⏳ Mesin Waktu DCA (Smart DCA)</h2>", unsafe_allow_html=True)
        ...
        if st.button("📏 Pasang Jaring Fibonacci", use_container_width=True):
            with st.spinner("Menarik garis matematika presisi tinggi..."):
                try:
                    ...
                    if df_hist.empty:
                        st.error(f"⚠️ Maaf, grafik {koin_prediksi} tidak bisa diakses.")
                        return
                    df_hist = df_hist.to_frame(name='Price')
                    ...
                    fig = go.Figure()
                    fig.add_trace(go.Scatter(x=df_hist['Date'], y=df_hist['Price'], mode='lines', name='Pergerakan Harga', line=dict(color='#38BDF8', width=2)))
...
def render_peta_kripto():
    ...
    fig.update_layout(margin=dict(t=30, l=0, r=0, b=0), paper_bgcolor="rgba(0,0,0,0)", font={'color': "white"})
    st.plotly_chart(fig, use_container_width=True)
            ...
            try:
                ...
                with urllib.request.urlopen(req, context=ctx, timeout=15) as response:
                    ...
                    for item in root.findall('./channel/item'):
                        ...
                        count += 1
                        if count >= 7: break
                    if count == 0: st.warning("Belum ada berita terbaru hari ini.")
            except Exception as e:
                try:
                    ...
                    st.error(f"Gagal menarik berita berbahasa Indonesia. Semua server target sedang sibuk.")
