...
# ==========================================================
# 2. MENU DOMPET TRADING LENGKAP (5 FITUR INSTITUSI)
# ==========================================================
def format_rp(val):
...
# Fungsi lokal untuk eksekusi Top Up Asset langsung ke Database
...
def render_dompet(user_now, role):
    ...
    # TAMBAHAN: TAB PASIF INCOME UNTUK DIVIDEN
    tab1, tab2, tab_passive, tab3 = st.tabs(["📈 KEPEMILIKAN", "📜 RIWAYAT (REALIZED)", "💰 PASIF INCOME", "📊 AUDIT JURNAL AI"])
    
    # AMBIL SEMUA HISTORY DULU UNTUK KALKULASI PASIF INCOME
    ...
    with tab1:
        ...
        if not df_p.empty:
            ...
            # --- DETEKSI CERDAS SAHAM VS KRIPTO ---
            ...
            def calc_active(r):
                ...
                if is_cr:
                    ...
                    fee_cost = (cost_rp * 0.0021) + (val_rp * 0.0021) # Fee Indodax 0.21%
                else:
                    ...
                    fee_cost = (cost_rp * 0.0015) + (val_rp * 0.0025) # Fee Saham Standar (0.15% Buy, 0.25% Sell)
            ...
            for _, r in df_p.iterrows():
                ...
                with st.expander(title):
                    ...
                    if tp_val > 0 and sl_val > 0 and tp_val > sl_val:
                        ...
                        posisi_clamp = max(0, min(100, posisi_skrng)) # Kunci di 0 - 100%
    ...
    with tab_passive:
        ...
        with st.form("form_passive_inc"):
            ...
            if st.form_submit_button("Catat Pemasukan", width="stretch") and sumber_dana:
                # Kita simpan di tabel history dengan strategi khusus agar tercatat sebagai Profit
...
# ==========================================
# 3. MENU DOKTER PORTOFOLIO & LAINNYA
# ==========================================
